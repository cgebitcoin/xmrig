 ...
 * <https://github.com/vtnerd> Copyright 2019      Howard Chu
 * <https://github.com/hyc> Copyright 2018-2024 SChernykh
 * <https://github.com/SChernykh> Copyright 2016-2024 XMRig
...
bool xmrig::Job::isEqual(const Job &other) const {
  return m_id == other.m_id && m_clientId == other.m_clientId &&
         isEqualBlob(other) && m_target == other.m_target;
}

bool xmrig::Job::isEqualBlob(const Job &other) const {
  return (m_size == other.m_size) &&
         (memcmp(m_blob, other.m_blob, m_size) == 0);
}
...
bool xmrig::Job::setTarget(const char *target) {
  static auto parse = [](const char *target, size_t size,
                         const Algorithm &algorithm) -> uint64_t {
    ...
    case 4:
      return 0xFFFFFFFFFFFFFFFFULL /
             (0xFFFFFFFFULL /
              uint64_t(*reinterpret_cast<const uint32_t *>(raw.data())));
...
#ifdef XMRIG_PROXY_PROJECT
  Cvt::toHex(m_rawTarget, sizeof(m_rawTarget),
             reinterpret_cast<uint8_t *>(&m_target), sizeof(m_target));
#endif
}
...
uint32_t xmrig::Job::getNumTransactions() const {
  ...
  // Monero (and some other coins) has the number of transactions encoded as
  // varint in the end of hashing blob
  const size_t expected_tx_offset =
      (m_algorithm == Algorithm::RX_WOW) ? 141 : 75;
...
void xmrig::Job::setMinerTx(const uint8_t *begin, const uint8_t *end,
                            size_t minerTxEphPubKeyOffset,
                            size_t minerTxPubKeyOffset,
                            size_t minerTxExtraNonceOffset,
                            size_t minerTxExtraNonceSize,
                            const Buffer &minerTxMerkleTreeBranch,
                            bool hasViewTag) {
...
void xmrig::Job::setExtraNonceInMinerTx(uint32_t extra_nonce) {
  memcpy(m_minerTxPrefix.data() + m_minerTxExtraNonceOffset, &extra_nonce,
         std::min(m_minerTxExtraNonceSize, sizeof(uint32_t)));
}

void xmrig::Job::generateSignatureData(String &signatureData,
                                       uint8_t &view_tag) const {
...
void xmrig::Job::generateHashingBlob(String &blob) const {
  ...
  BlockTemplate::calculateRootHash(p, p + m_minerTxPrefix.size(),
                                   m_minerTxMerkleTreeBranch, root_hash);
  ...
  blob = rawBlob();
  Cvt::toHex(blob.data() + root_hash_offset * 2, 64, root_hash,
             BlockTemplate::kHashSize);
}

#else

void xmrig::Job::generateMinerSignature(const uint8_t *blob, size_t size,
                                        uint8_t *out_sig) const {
  ...
  xmrig::generate_signature(prefix_hash, m_ephPublicKey, m_ephSecretKey,
                            out_sig);
}
